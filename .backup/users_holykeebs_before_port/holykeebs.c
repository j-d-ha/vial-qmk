...
static void hk_apply_pointer_kind_defaults(hk_pointer_state_t* state) {
    switch (state->pointer_kind) {
        case POINTER_KIND_TRACKPOINT:
            hk_configure_trackpoint_common(state);
            break;
        case POINTER_KIND_CIRQUE35:
        case POINTER_KIND_CIRQUE40:
            hk_configure_cirque_common(state);
            break;
        case POINTER_KIND_TPS43:
            hk_configure_tps43_common(state);
            break;
        case POINTER_KIND_PIMORONI_TRACKBALL:
            hk_configure_pimoroni_common(state);
            break;
        default:
            break;
    }
...
static void hk_sanitize_pointer_state(hk_pointer_state_t* state, bool is_peripheral) {
    hk_pointer_state_t defaults = {.pointer_kind = state->pointer_kind};
    hk_apply_pointer_kind_defaults(&defaults);

    if (state->cursor_mode > CURSOR_MODE_SNIPING) {
        state->cursor_mode = CURSOR_MODE_DEFAULT;
    }
    if (state->scroll_lock > SCROLL_LOCK_VERTICAL) {
        state->scroll_lock = SCROLL_LOCK_OFF;
    }

    if (state->pointer_default_multiplier <= 0 || state->pointer_default_multiplier > 8.0f) {
        state->pointer_default_multiplier = defaults.pointer_default_multiplier;
    }
    if (state->pointer_sniping_multiplier <= 0 || state->pointer_sniping_multiplier > 8.0f) {
        state->pointer_sniping_multiplier = defaults.pointer_sniping_multiplier;
    }
    if (state->pointer_scroll_buffer_size > 32) {
        state->pointer_scroll_buffer_size = defaults.pointer_scroll_buffer_size;
    }

    if (is_peripheral && state->pointer_kind == POINTER_KIND_PIMORONI_TRACKBALL) {
        state->drag_scroll = true;
    }
}
...
void keyboard_post_init_user(void) {
    ...
    printf("keyboard_post_init_user: reading eeprom, magic: %u, version: %u\n", hk_eeprom_config.magic, hk_eeprom_config.version);
    if (!eeconfig_is_user_datablock_valid() || hk_eeprom_config.magic != HK_EEPROM_MAGIC || hk_eeprom_config.version < HK_EEPROM_VERSION) {
        printf("keyboard_post_init_user: eeprom data not found or outdated, initializing\n");
        eeconfig_init_user();
    } else {
        g_hk_state = init_state();
        deserialize_eeconfig_to_state(&hk_eeprom_config);
        hk_sanitize_pointer_state(&g_hk_state.main, false);
        hk_sanitize_pointer_state(&g_hk_state.peripheral, true);
        debug_hk_state_to_console(&g_hk_state);
    }
...
void                       eeconfig_init_user(void) {
    ...
    memset(&hk_eeprom_config, 0, sizeof(hk_eeprom_config_t));
    hk_eeprom_config.magic = HK_EEPROM_MAGIC;
    hk_eeprom_config.version = HK_EEPROM_VERSION;
    serialize_state_to_eeconfig(&hk_eeprom_config);
